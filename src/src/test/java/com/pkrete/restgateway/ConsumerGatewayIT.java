...
public class ConsumerGatewayIT extends TestCase {
    ...
    public void testConsumerGateway1Json() {
        ...
        assertEquals(result, new String(data.getBytes(), StandardCharsets.UTF_8));
    ...
    public void testConsumerGateway4Json() {
        ...
        String url = "api.kirjastot.fi/v3/organisation/";
        Map<String, String> headers = new HashMap<>();
        headers.put("Accept", "application/json");
        ...
        List<String> values = new ArrayList<>();
        values.add("kallio");
        urlParams.put("name", values);

        values = new ArrayList<>();
        values.add("helsinki");
        urlParams.put("city.name", values);
